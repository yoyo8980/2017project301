...
public class Main {
    ...
    public void test(String accessToken) {
        // access token 지정
        apiHelper.setAccessToken(accessToken);
        System.out.println("액세스토큰 백으로 잘 넘어옴ㅎ"+accessToken);

        // 푸시 알림이나 유저 아이디 리스트가 필요할 때 설정 합니다. 
        // (디벨로퍼스 내에 앱설정 메뉴를 가시면 있습니다)
        apiHelper.setAdminKey("a263d80788886d627dbf8517d5a52c52");
...
void testUserManagement() {
	...
	public String login() {
		 ...
		 // 앱 연결 
         apiHelper.signup(); //로그인할때마다 연결하게 됨;
        System.out.println("연결됨");
